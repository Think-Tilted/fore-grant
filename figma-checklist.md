...
- [x] **Row 8 — Hole assignments copy** — "Update the Hole Assignments section copy. Remove the helper text 'Groups are posted the morning of the tournament.' from the search area. In the gray lower search results section, replace the existing message with: 'Groups and hole assignments will be posted closer to the event.'" Fixed: both copy changes applied exactly.
- [x] **Row 9 — Confirmation modal** — "In the Confirm Registration modal after clicking Review, replace the light red highlights around the contact info with a single orange callout box. Red should be reserved for errors. Use one orange outline around the entire contact review section, with all three lines contained in the same box." Fixed: JS now groups all contact rows into a single `confirm-contact-callout` div with `2px solid accent-display` orange border; removed individual `confirm-row-highlight` per-row backgrounds.
- [x] **Row 10 — Special requests on mobile** — "Add a line break after 'Requests' in the 'Special Requests or Comments' header so the text stays within the card width. Mobile only." Fixed: wrapped " or Comments" in a `<span class="section-title-break">` with `display: block` at `max-width: 767px` on both sponsor and foursome forms.
- [x] **Row 11 — Form fields on mobile** — "Adjust the form and/or card widths so the page only scrolls vertically. This may be related to the header text issue above or the cards being too wide." Reverted section-num/section-title changes — they made it worse. Row 10's section-title-break fix likely resolves the root cause; needs re-evaluation in the browser.
- [x] **Row 12 — Confirmation page post-modal** — "On the 'You're Registered' page, add an orange underline beneath 'You're Registered.' Reduce the size of the 'Your Package' and 'What Happens Next' subheaders to create stronger visual hierarchy and improve the mobile experience." Fixed: orange underline on h1; subheadings reduced to heading-bucket; 'Your package' label hidden on mobile; price scaled down on mobile; 'What happens next' numbers changed to large orange accent-display matching Figma; step copy updated to match EMAIL-SPEC. Also fixed tier pre-selection on sponsor form (JS reads data-tier-id attr) and confirmation page data (JSON island + confirmation.js hydrates receipt at runtime).
